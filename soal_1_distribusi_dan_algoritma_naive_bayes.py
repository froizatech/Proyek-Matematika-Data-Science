...
Original file is located at
    https://colab.research.google.com/drive/1sAU8ajQtgwJS_plqSPne3LYXqdbEAjTF
...
def viz_uniform_teoretis(lower_bound, upper_bound, num_samples):
  ...
  a = generate_rand_uniform(lower_bound, upper_bound, num_samples)
  ...
  plt.hist(a, bins=30, edgecolor="black", color="lightgrey", density=True)
...
def uniform_inverse_cdf(probability, miu, sigma):
  ...
  x = sigma * np.sqrt(2) * erfinv(2*p - 1) + miu
...
def viz_distribusi_gaussian(first_gaussian, second_gaussian, third_gaussian):
  ...
  lb = "first_gaussian"
  x = first_gaussian
  plt.hist(x, alpha=0.5, bins=32, label = lb)

  lb = "second_gaussian"
  y = second_gaussian
  plt.hist(y, alpha=0.5, bins=32, label=lb)

  lb = "third_gaussian"
  z = third_gaussian
  plt.hist(z, alpha=0.5, bins=32, label=lb)
...
def generate_binomial(n_trials, success_prob, num_samples):
  ...
  array = binomial_inverse_cdf(probability, n_trials, success_prob)
  # AKHIRI KODE DI SINI
...
def viz_distribusi_binomial(first_binomial, second_binomial, third_binomial):
  ...
  lb = "first_binomial"
  x = first_binomial
  plt.hist(x, alpha=0.5, label = lb)

  lb = "second_binomial"
  y = second_binomial
  plt.hist(y, alpha=0.5, label=lb)

  lb = "third_binomial"
  z = third_binomial
  plt.hist(z, alpha=0.5, label=lb)
...
def generate_data_synthetic(breed, features, n_samples, params):
  ...
  for feature in features:
      ...
      if feature == ["wingspan_cm","weight_g"] :
      ...
      if feature == ["sing_days"] :
        df[feature] = generate_binomial(
            n_trials=paramse.n_trials, success_prob=paramse.success_prob,
            num_samples=num_samples
              ...
              # Simpan hasil generasi pada kolom dataframe sesuai dengan nama fitur
      if feature == ["beak_head_ratio"] :
        df[feature] = generate_rand_uniform(lower_bound=paramse.lower_bound, upper_bound=paramse.upper_bound,
        num_samples=num_samples
            )
...
def pdf_for_gaussian(val, miu, sigma):
  ...
  # MULAI KODE DI SINI
  p = np.asarray(val)
  pdf = (1/(sigma*np.sqrt(2*np.pi)))*np.exp(-0.5*((p-miu)/sigma)**2)
  # AKHIRI KODE DI SINI
...
def pmf_for_binomial(val, n_trials, probability):
  ...
  # MULAI KODE DI SINI
  k = np.asarray(val)
  pmf = binom.pmf(k, n_trials, probability)
  # AKHIRI KODE DI SINI
...
def binomial_params_estimation(sample):
  ...
  # MULAI KODE DI SINI
  success_probability = sample.mean() / n_trials
  # AKHIRI KODE DI SINI
...
def uniform_params_estimation(sample):
  ...
  """
  # MULAI KODE DI SINI
  lower_bound = sample.min()
  upper_bound = sample.max()
  # AKHIRI KODE DI SINI
